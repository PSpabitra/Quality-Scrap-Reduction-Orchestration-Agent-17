...
export default function RCAPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-display text-white">AI Root Cause Analysis</h1>
      ...
      {selResult && (
        <div className="card p-4 flex flex-wrap gap-6 text-sm">
          <div><span className="label">Driver</span><div className="text-white">{selResult.component_code} · {selResult.defect_code} · {selResult.machine}</div></div>
          <div><span className="label">Scrap %</span><div>{selResult.scrap_percent?.toFixed(2)}%</div></div>
          <div><span className="label">COPQ</span><div className="text-amber-400">₹{Math.round(selResult.copq).toLocaleString()}</div></div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="space-y-6">
            <div className="card p-5">
              ...
              <p className="text-slate-200 text-sm leading-relaxed">{rca.ai_summary}</p>
              <div className="mt-3 text-xs text-slate-500">Confidence: <span className="text-cyan-300">{(rca.confidence_score * 100).toFixed(0)}%</span>{rca.confidence_score <= 0.2 && <span className="ml-2 text-amber-400">(low confidence / fallback)</span>}</div>
            </div>
            <div className="card p-5">
              <div className="label mb-1">Root Cause</div>
              <p className="text-slate-200 text-sm leading-relaxed">{rca.ai_root_cause}</p>
            </div>
            <div className="card p-5">
              <div className="label mb-1">Recommended Solution</div>
              <p className="text-slate-200 text-sm leading-relaxed">{rca.ai_solution}</p>
            </div>
            <div className="card p-5">
              <div className="label mb-2">Evidence</div>
              <ul className="space-y-2">
                {rca.evidence?.map((e: string, i: number) => (
                  <li key={i} className="text-sm text-slate-300 pl-3 border-l-2 border-cyan-500/40">{e}</li>
                ))}
              </ul>
            </div>
          </div>
          <div className="card p-5 h-fit">
            ...
            <div className="space-y-3">
              {rca.recommended_actions?.map((a: any, i: number) => (
                <div key={i} className="border border-edge rounded-lg p-4 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div className="text-white font-medium text-sm">{a.title}</div>
                    <span className="text-xs px-2 py-0.5 rounded bg-edge text-cyan-300">{a.priority}</span>
                  </div>
      ...
      {!rca && sel != null && <div className="card p-6 text-slate-500 text-sm">No RCA yet for this driver — click Generate RCA.</div>}
    </div>
  );
}
